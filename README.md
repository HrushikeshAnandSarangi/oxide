...
- **Graceful shutdown** — SIGTERM/Ctrl+C stop the API from accepting new connections (finishing in-flight ones, up to a 20s bound), stop the health monitor and deployment worker from starting new work, and stop the telemetry consumer cleanly; the Pingora proxy drains its own connections via its built-in signal handling, configured to a 10s grace period instead of its 300s default (see [proxy/src/proxy.rs](proxy/src/proxy.rs)). Live-verified with real SIGTERM against running processes, not just read from the code.
